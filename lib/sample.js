...
function vote(tx) {
    ...
    if(allVotes && allVotes.indexOf(originP) >= 0){
        throw new Error('Do not vote twice for the same NewsItem');
    }
    if(tx.newsItem.author == originP){
       throw new Error('Do not vote for your own NewsItem');
    }

    // Store the voter in our list
    if (!tx.newsItem.voters){
      tx.newsItem.voters = [];
    }
